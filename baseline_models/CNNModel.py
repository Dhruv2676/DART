...
class TqdmCallback(tf.keras.callbacks.Callback):
    """
    Custom Keras callback to display a tqdm progress bar for epochs.
    Updates the bar at the end of each epoch with loss and accuracy metrics.
    """
    ...
    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}
        # Try to get val_f1_macro, default to val_accuracy if not present
...
class CNNModel:
    def __init__(self):
        """
        Initializes the CNN model parameters and hyperparameters.
        """
        self.lookback = 50
        self.num_features = 1
    ...
    def build_model(self):
        ...
        Layers: Conv1D -> MaxPooling1D -> Conv1D -> MaxPooling1D -> Flatten -> Dense (Softmax)
        """
        self.model = models.Sequential(name='CNN')
        input_shape = (self.lookback, self.num_features)
        
        for i, conv in enumerate(self.hyperparameters['conv_layers']):
            self.model.add(layers.Conv1D(filters=conv['filters'], kernel_size=conv['kernel_size'], 
                                     activation='relu', input_shape=input_shape, name=f'conv{i+1}'))
            self.model.add(layers.MaxPooling1D(pool_size=self.hyperparameters['pool_size'], name=f'pool{i+1}'))
        ...
        self.model.compile(optimizer=optimizers.Adam(learning_rate=self.hyperparameters['learning_rate']),
                           loss='categorical_crossentropy', metrics=['accuracy', metrics.F1Score(average='macro', name='f1_macro'), metrics.F1Score(average='micro', name='f1_micro')])
    ...
    def train(self, X_train, y_train, X_val, y_val, run_idx=0):
        ...
        - run_idx: Index of the current run (used for logging verbosity).
    ...
    def evaluate(self, X_test, y_test, run_idx=0):
        ...
        Returns:
        - results: List of evaluation metrics [loss, accuracy, f1_macro, f1_micro].
        """
        if self.model is None:
            return None
...
def get_split_data(dataset, split='train'):
    """
    Loads and reshapes data from the dataset class for a specific split.
    Flattens the timesteps and companies into a single batch for CNN training.
    
    Returns:
    - X: Reshaped features array (batch_size, lookback, num_features).
    - y: Reshaped labels array (batch_size, num_labels).
    """
    if split == 'train':
...
def main():
    ...
    """
    print("=" * 80)
    print(f"CNN Model Training - Averaging {NUM_RUNS} runs per phase")
    print("=" * 80)
    ...
    overall_results = {
        'model_name': cnn.hyperparameters['model_name'],
    ...
    for phase_idx in range(num_phases):
        ...
        for run_idx in range(NUM_RUNS):
            ...
            history = cnn.train(X_train, y_train, X_val, y_val, run_idx)
            training_time = time.time() - start_time
            
            test_results = cnn.evaluate(X_test, y_test, run_idx)
            ...
            if backtest_data_ready:
                ...
                test_predictions = cnn.model.predict(X_test, batch_size=cnn.hyperparameters['batch_size'], verbose=0)
    ...
    log_file_path = os.path.join(log_dir, 'CNNModel_overall_result.json')
