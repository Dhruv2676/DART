...
class LSTMModel:
    ...
    def build_model(self):
        ...
        Layers: LSTM (return_seq=True) -> Dropout -> BatchNorm -> LSTM (return_seq=False) -> Dropout -> BatchNorm -> Dense.
        """
        self.model = models.Sequential(name='LSTM')
        input_shape = (self.lookback, self.num_features)
        
        # First LSTM layer returns sequences to stack with the second layer
        self.model.add(layers.LSTM(self.hyperparameters['lstm_layers'][0], return_sequences=True, input_shape=input_shape, name='lstm_1', recurrent_dropout=0.2))
        self.model.add(layers.Dropout(self.hyperparameters['dropout_rate']))
        self.model.add(layers.BatchNormalization())
        
        # Second LSTM layer returns only the final state vector
        self.model.add(layers.LSTM(self.hyperparameters['lstm_layers'][1], return_sequences=False, name='lstm_2', recurrent_dropout=0.2))
        self.model.add(layers.Dropout(self.hyperparameters['dropout_rate']))
        self.model.add(layers.BatchNormalization())
        
        self.model.add(layers.Dense(self.num_labels, activation='softmax', name='output'))
        
        self.model.compile(optimizer=optimizers.Adam(learning_rate=self.hyperparameters['learning_rate'], clipnorm=1.0),
    ...
    def train(self, X_train, y_train, X_val, y_val, run_idx=0):
        """
        Trains the LSTM model with callbacks for early stopping and learning rate reduction.
        """
        ...
        reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_f1_macro', factor=0.5, patience=7, mode='max', min_lr=1e-6, verbose=0)
...
def get_split_data(dataset, split='train'):
    """
    Extracts and reshapes data for the LSTM model.
    Flattens the company dimension so the LSTM processes each company's time series independently.
    Output Shape: (Total Samples, Lookback, Features)
    """
    if split == 'train':
...
def main():
    ...
    """
    print("=" * 80)
    print(f"LSTM Model Training - Averaging {NUM_RUNS} runs per phase")
    print("=" * 80)
    ...
    overall_results = {
        'model_name': lstm.hyperparameters['model_name'],
    ...
    for phase_idx in range(num_phases):
        ...
        for run_idx in range(NUM_RUNS):
            ...
            history = lstm.train(X_train, y_train, X_val, y_val, run_idx)
            training_time = time.time() - start_time
            
            test_results = lstm.evaluate(X_test, y_test, run_idx)
            ...
            if backtest_data_ready:
                ...
                test_predictions = lstm.model.predict(X_test, batch_size=lstm.hyperparameters['batch_size'], verbose=0)
    ...
    log_file_path = os.path.join(log_dir, 'LSTMModel_overall_result.json')
